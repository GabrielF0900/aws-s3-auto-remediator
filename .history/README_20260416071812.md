...
| | |
|---|---|
| **O que é** | Orquestrador serverless de remediação automática de segurança para buckets S3 |
| **Problema Resolvido** | Buckets S3 criados com acesso público por padrão, eliminando riscos de exposição de dados e violações de compliance |
| **Stack e AWS** | **Node.js** • **Lambda** • **EventBridge** • **CloudTrail** • **S3** • **IAM** |
| **Resultado Prático** | Detecção e bloqueio de acesso público em tempo real, reduzindo o tempo de remediação de horas para segundos com custo operacional próximo a zero (pay-per-use) |
